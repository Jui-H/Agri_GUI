...
async function handleSamplingDone() {
  ...
  /*
    Display-only sprinkler timing.
    SAMPLE,DONE finalizes the prescription.
    The farmer operates the pump manually.
  */
  ...
  if (!area) {
    ...
    setText("selectedCropTarget", "-- সেকেন্ড");
    return;
  }
  ...
  setText(
    "selectedCropTarget",
    `${toBanglaNumber(totalTimeSec.toFixed(2))} সেকেন্ড`
  );

  toast(
    `সার দেওয়ার সময়: ${toBanglaNumber(totalTimeSec.toFixed(2))} সেকেন্ড`
  );
  ...
  if (latestArea) {
    ...
    setText(
      ...
      Number.isFinite(totalTime) && totalTime > 0
        ? `${toBanglaNumber(totalTime.toFixed(2))} সেকেন্ড`
        : "-- সেকেন্ড"
    );
...
async function startAutonomousFertilization() {
  ...
  if (!area || !calibrationReady()) {
    toast("Sprinkler time is not ready yet");
    return;
  }
