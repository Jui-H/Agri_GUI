...
/* =========================================================
   ...
   Treatment area is fixed at 6 × 6 ft.
   Measured liquid flow is fixed at 2 L/min = 2000 mL/min.
   Only nutrient concentration (g/L) must be entered.
========================================================= */

const FERTILIZER_FLOW_L_MIN = 2.0;
const FERTILIZER_FLOW_ML_MIN = 2000;
const FERTILIZER_FLOW_ML_SEC = FERTILIZER_FLOW_ML_MIN / 60;

// Prevent the same completed area from being fertilized twice.
let lastAutoFertilizedAreaIndex = 0;
...
function processSerialLine(
  ...
  const normalizedSystemLine =
    line.replace(/\s+/g, "").toUpperCase();

  if (
    normalizedSystemLine === "SAMPLE,DONE" ||
    normalizedSystemLine === "ROVER,SAMPLE,DONE" ||
    line.includes(
      "SAMPLING_DONE"
    )
  ) {
...
function updateLiveTelemetry(
  ...
  */
  ...
  /* GUI-calculated required N/P/K are refreshed from completed 3-sample areas. */
...
/* =========================================================
   ...
   Every three valid positive NPK samples are grouped internally
...
function collectMissionSample(data) {
  ...
  /*
    STRICT AUTOMATIC SAMPLING RULE

    A telemetry packet counts as ONE sample when:
      1. A crop is selected, and
      2. N > 0, P > 0, K > 0.

    Nothing else affects sample acceptance:
      - GPS is NOT required.
      - Duplicate NPK values are allowed.
      - Packet number is NOT used as a filter.
      - 0,0,0 is invalid.
      - Any individual nutrient <= 0 makes the packet invalid.

    Every 3 accepted samples automatically generate one
    fertilization area and update the deficiency heatmap.
  */
  ...
  missionSamples.push({
    ...
    // GPS is metadata only. It has no role in heatmap sampling.
...
function rebuildFertilizationAreas() {
  ...
  const valid = missionSamples.filter(sample =>
    Number.isFinite(sample.measuredN) &&
    Number.isFinite(sample.measuredP) &&
    Number.isFinite(sample.measuredK) &&
    sample.measuredN > 0 &&
    sample.measuredP > 0 &&
    sample.measuredK > 0
  );
  ...
  for (let i = 0; i + SAMPLES_PER_FERTILIZATION_AREA - 1 < valid.length; i += SAMPLES_PER_FERTILIZATION_AREA) {
    ...
    const areaIndex = fertilizationAreas.length + 1;

    /*
      Heatmap placement is sequence-based, not GPS-based.
      Areas are arranged in a compact serpentine grid purely
      for indoor visualization.

      Area 1  Area 2  Area 3
      Area 6  Area 5  Area 4
      Area 7  Area 8  Area 9 ...
    */
    const HEATMAP_COLUMNS = 3;
    const zeroBasedArea = areaIndex - 1;
    const gridRow = Math.floor(zeroBasedArea / HEATMAP_COLUMNS);
    const positionInRow = zeroBasedArea % HEATMAP_COLUMNS;
    const gridColumn =
      gridRow % 2 === 0
        ? positionInRow
        : (HEATMAP_COLUMNS - 1 - positionInRow);

    const area = {
      areaIndex,
      samples,

      // Optional GPS is retained only as metadata.
      latitude: average("latitude"),
      longitude: average("longitude"),

      // Synthetic local coordinates used by the heatmap.
      heatmapX: gridColumn * FERTILIZATION_AREA_SIZE_FT,
      heatmapY: gridRow * FERTILIZATION_AREA_SIZE_FT,
...
function calculateAreaApplication(area) {
  ...
  const nTime = onTimeSeconds(nVolume, FERTILIZER_FLOW_ML_MIN);
  const pTime = onTimeSeconds(pVolume, FERTILIZER_FLOW_ML_MIN);
  const kTime = onTimeSeconds(kVolume, FERTILIZER_FLOW_ML_MIN);

  const volumes = [nVolume, pVolume, kVolume];

  const totalVolumeMl =
    volumes.every(value => Number.isFinite(value))
      ? volumes.reduce((sum, value) => sum + value, 0)
      : null;

  // One sprinkler/pump command for the complete prepared solution.
  const totalTimeSec = onTimeSeconds(totalVolumeMl, FERTILIZER_FLOW_ML_MIN);
...
function convertSamplesToLocalXY(
  ...
  return samples.map(
    (sample, index) => {
      ...
      // Indoor heatmap: use sequence/grid coordinates when available.
      if (
        Number.isFinite(sample.heatmapX) &&
        Number.isFinite(sample.heatmapY)
      ) {
        return {
          ...sample,
          x: sample.heatmapX,
      ...
      // GPS remains a fallback only, never a sampling requirement.
      if (
        validGps(sample.latitude, sample.longitude) &&
        ...
        return {
          ...sample,
          x:
            (sample.longitude - originLon) *
            metersPerLonDegree,
          y:
            (sample.latitude - originLat) *
            metersPerLatDegree
        };
      }

      // Final fallback: sequential placement.
      return {
        ...sample,
        x: index * SAMPLING_AREA_SIZE_FT,
        y: 0
      };
    }
  );
...
function renderFieldHeatmap(
  ...
  const sourcePoints =
    state.selectedCrop?.isSurvey
      ? missionSamples.map((sample, index) => {
          const HEATMAP_COLUMNS = 3;
          const row = Math.floor(index / HEATMAP_COLUMNS);
          const pos = index % HEATMAP_COLUMNS;
          const col =
            row % 2 === 0
              ? pos
              : (HEATMAP_COLUMNS - 1 - pos);

          return {
            ...sample,
            heatmapX: col * SAMPLING_AREA_SIZE_FT,
            heatmapY: row * SAMPLING_AREA_SIZE_FT,
            defN: 0,
            defP: 0,
            defK: 0
          };
  ...
  const firstGpsSample =
    missionSamples.find(sample =>
      validGps(sample.latitude, sample.longitude)
    );

  const commonOriginLat =
    firstGpsSample?.latitude ?? 0;

  const commonOriginLon =
    firstGpsSample?.longitude ?? 0;
...
async function handleSamplingDone() {
  ...
  /*
    Autonomous fertilization trigger:
    Rover -> LoRa -> base station -> USB Serial -> GUI : SAMPLE,DONE

    At this point the GUI has already calculated the prescription.
    It now immediately sends the required sprinkler ON time:
      SPRINKLER,<milliseconds>
  */

  const area = latestFertilizationArea();

  if (!area) {
    setFertilizationState("WAITING FOR 3 NPK SAMPLES");
    toast("SAMPLE,DONE received, but no complete 3-sample area exists");
    return;
  }

  if (area.areaIndex <= lastAutoFertilizedAreaIndex) {
    return;
  }

  if (!calibrationReady()) {
    setFertilizationState("CALIBRATION REQUIRED");
    toast("SAMPLE,DONE received, but fertilizer concentration is not set");
    return;
  }

  const totalTimeSec = area.application?.totalTimeSec;

  if (!Number.isFinite(totalTimeSec) || totalTimeSec <= 0) {
    setFertilizationState("INVALID ON TIME");
    toast("SAMPLE,DONE received, but sprinkler ON time is invalid");
    return;
  }
  ...
  setFertilizationState(
    `SENDING ${totalTimeSec.toFixed(2)} s`
  );

  const sent =
    await sendBaseCommand(command);

  if (!sent) {
    setFertilizationState("SEND FAILED");
    return;
  }

  lastAutoFertilizedAreaIndex =
    area.areaIndex;

  setFertilizationState(
    `AUTO FERTILIZING • ${totalTimeSec.toFixed(2)} s`
  );
  ...
  toast(
    `SAMPLE,DONE → sprinkler ON for ${totalTimeSec.toFixed(2)} s`
  );
...
function updatePrescription() {
  ...
  if (latestArea) {
    ...
    const totalVolume = latestArea.application.totalVolumeMl;
    const totalTime = latestArea.application.totalTimeSec;

    setText("areaTotalVolume", formatMaybe(totalVolume, 2, " mL"));
    setText("areaTotalTime", formatMaybe(totalTime, 2, " s"));

    setText("calibrationState", calibrationReady() ? "READY TO APPLY" : "Calibration required");

    if (calibrationReady() && Number.isFinite(totalTime)) {
      setFertilizationState("PRESCRIPTION READY");
    }
  }
...
async function startAutonomousFertilization() {
  ...
  if (!area) {
    toast("Three valid positive NPK samples are required first");
    return;
  }

  if (!calibrationReady()) {
    setFertilizationState("CALIBRATION REQUIRED");
    toast("Enter N, P and K nutrient concentration before application");
    return;
  }
  ...
  if (!Number.isFinite(totalTimeSec) || totalTimeSec <= 0) {
    setFertilizationState("INVALID ON TIME");
    toast("Unable to calculate a valid sprinkler ON time");
    return;
  }

  const onTimeMs = Math.max(1, Math.round(totalTimeSec * 1000));

  /*
    Command path:
    GUI -> USB Serial -> base-station ESP32 -> GPIO32 relay -> solenoid valve

    Base-station firmware should handle:
      SPRINKLER,<milliseconds>
  */
  const command = `SPRINKLER,${onTimeMs}`;

  const sent = await sendBaseCommand(command);
  if (!sent) return;

  setFertilizationState(`COMMAND SENT • ${(onTimeMs / 1000).toFixed(2)} s`);
  updateSequence("FERTILIZING");

  toast(`Fertilizer command sent: ${(onTimeMs / 1000).toFixed(2)} s`);
}
...
function populateCalibrationInputs() {
  ...
  setText("fixedFlowRateDisplay", `${FERTILIZER_FLOW_L_MIN.toFixed(1)} L/min`);
  setText("calibrationState", calibrationReady() ? "READY TO APPLY" : "Calibration required");
  setText("calibrationStateSprinkler", calibrationReady() ? "Concentration saved" : "Enter concentration");
}
...
function saveFertilizerCalibration() {
  ...
  toast(
    calibrationReady()
      ? "Fertilizer concentration saved • flow fixed at 2 L/min"
      : "Enter positive N, P and K nutrient concentrations"
  );
}
